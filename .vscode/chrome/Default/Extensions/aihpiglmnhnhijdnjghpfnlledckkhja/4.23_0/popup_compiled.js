...
var doc$jscomp$inline_16 = goog.global.document;
JSCompiler_inline_result$jscomp$11 = doc$jscomp$inline_16 && goog.userAgent.IE ? goog.userAgent.getDocumentMode_() || ("CSS1Compat" == doc$jscomp$inline_16.compatMode ? parseInt(goog.userAgent.VERSION, 10) : 5) : void 0;
goog.userAgent.DOCUMENT_MODE = JSCompiler_inline_result$jscomp$11;
...
bcx.polyjuice.PolyjuiceOptions = function(options) {
  this.options_ = options;
  this.networkName_ = this.countryCode_ = "";
  this.requireStealthy_ = this.requireMobile_ = !1;
};
bcx.polyjuice.PolyjuiceOptions.log_ = function(fn, entry) {
  bcx.Logger.get().log("bcx.PolyjuiceOptions", fn, entry);
};
bcx.polyjuice.PolyjuiceOptions.prototype.setSelectedCountryCode = function(countryCode) {
  this.countryCode_ = countryCode;
};
bcx.polyjuice.PolyjuiceOptions.prototype.setSelectedNetworkName = function(networkName) {
  this.networkName_ = networkName;
};
bcx.polyjuice.PolyjuiceOptions.prototype.setRequireMobile = function(require) {
  this.requireMobile_ = require;
};
bcx.polyjuice.PolyjuiceOptions.prototype.setRequireStealthy = function(require) {
  this.requireStealthy_ = require;
};
bcx.polyjuice.PolyjuiceOptions.prototype.getCountries = function() {
  var networkName = this.networkName_, requireMobile = this.requireMobile_, requireStealthy = this.requireStealthy_, countrySet = new Set;
  goog.array.forEach(this.options_, function(option) {
    if (!networkName || option.networkName === networkName) {
      if (!requireMobile || option.isMobile) {
        requireStealthy && option.isBurnt || option.country && countrySet.add(option.country);
      }
    }
  });
  return Array.from(countrySet).sort();
};
bcx.polyjuice.PolyjuiceOptions.prototype.getNetworks = function() {
  var countryCode = this.countryCode_, requireMobile = this.requireMobile_, requireStealthy = this.requireStealthy_, networkSet = new Set;
  goog.array.forEach(this.options_, function(option) {
    if (!countryCode || option.country === countryCode) {
      if (!requireMobile || option.isMobile) {
        requireStealthy && option.isBurnt || option.networkName && networkSet.add(option.networkName);
      }
    }
  });
  return Array.from(networkSet).sort();
};
bcx.polyjuice.PolyjuiceOptions.prototype.getStealthyOptions = function() {
  var countryCode = this.countryCode_, networkName = this.networkName_, requireMobile = this.requireMobile_, found = {stealthy:!1, nonStealthy:!1};
  goog.array.forEach(this.options_, function(option) {
    if (!countryCode || option.country === countryCode) {
      if (!networkName || option.networkName === networkName) {
        if (!requireMobile || option.isMobile) {
          option.isBurnt ? found.nonStealthy = !0 : found.stealthy = !0;
        }
      }
    }
  });
...
bcx.polyjuice.PolyjuiceOptions.prototype.getMobileOptions = function() {
  var countryCode = this.countryCode_, networkName = this.networkName_, requireStealthy = this.requireStealthy_, found = {mobile:!1, nonMobile:!1};
  goog.array.forEach(this.options_, function(option) {
    countryCode && option.country !== countryCode || networkName && option.networkName !== networkName || requireStealthy && option.isBurnt || (option.isMobile ? found.mobile = !0 : found.nonMobile = !0);
  });
  return found;
};
goog.async = {};
...
bcx.Storage = function() {
  ...
  this.showChina_ = !1;
  this.polyjuiceNetwork_ = this.polyjuiceCountry_ = "";
  this.polyjuiceUseStealthy_ = this.polyjuiceUseMobile_ = !1;
...
bcx.Storage.prototype.handleLocalStorageReply_ = function(resp) {
  ...
  if (e = resp[bcx.consts.SELECTED_NETWORK]) {
    this.polyjuiceNetwork_ = e;
  }
  if (e = resp[bcx.consts.SELECTED_MOBILE]) {
    this.polyjuiceUseMobile_ = e;
  }
  if (e = resp[bcx.consts.SELECTED_UNBURNT]) {
    this.polyjuiceUseStealthy_ = e;
  }
...
bcx.Storage.prototype.set = function(o) {
  ...
  void 0 != o[bcx.consts.SELECTED_NETWORK] && (this.polyjuiceNetwork_ = o[bcx.consts.SELECTED_NETWORK]);
  void 0 != o[bcx.consts.SELECTED_MOBILE] && (this.polyjuiceUseMobile_ = o[bcx.consts.SELECTED_MOBILE]);
  void 0 != o[bcx.consts.SELECTED_UNBURNT] && (this.polyjuiceUseStealthy_ = o[bcx.consts.SELECTED_UNBURNT]);
...
goog.iter.forEach = function(iterable, f, opt_obj) {
  if (goog.isArrayLike(iterable)) {
    ...
    try {
      ...
      if (ex$9 !== goog.iter.StopIteration) {
        throw ex$9;
      }
    }
  }
...
goog.window.open = function(linkRef, opt_options, opt_parentWin) {
  ...
  var safeLinkRef = linkRef instanceof goog.html.SafeUrl ? linkRef : goog.html.SafeUrl.sanitize("undefined" != typeof linkRef.href ? linkRef.href : String(linkRef));
...
goog.window.openBlank = function(opt_message, opt_options, opt_parentWin) {
  var loadingMessage = opt_message ? goog.string.escapeString(goog.string.htmlEscape(opt_message)) : "";
...
goog.dom.forms.getFormDataHelper_ = function(form, result, fnAppend) {
  for (var els = form.elements, el, i = 0; el = els[i]; i++) {
    if (el.form == form && !el.disabled && "FIELDSET" != el.tagName) {
      var name = el.name;
...
goog.dom.forms.getValue = function(el) {
  ...
  switch(goog.isString(type) && type.toLowerCase()) {
    ...
    default:
      return null != el.value ? el.value : null;
  }
};
...
goog.dom.forms.setValue = function(el, opt_value) {
  ...
  switch(goog.isString(type) && type.toLowerCase()) {
    ...
    case goog.dom.InputType.SELECT_MULTIPLE:
      ...
      break;
    default:
      el.value = null != opt_value ? opt_value : "";
  }
};
...
BD:"Bangladesh", BE:"Belgium", BF:"Burkina Faso", BG:"Bulgaria", BH:"Bahrain", BI:"Burundi", BJ:"Benin", BL:"St. Barth\u00e9lemy", BM:"Bermuda", BN:"Brunei", BO:"Bolivia", BQ:"Caribbean Netherlands", BR:"Brazil", BS:"Bahamas", BT:"Bhutan", BV:"Bouvet Island", BW:"Botswana", BY:"Belarus", BZ:"Belize", CA:"Canada", CC:"Cocos (Keeling) Islands", CD:"Congo - Kinshasa", CF:"Central African Republic", CG:"Congo - Brazzaville", CH:"Switzerland", CI:"C\u00f4te d\u2019Ivoire", CK:"Cook Islands", CL:"Chile", 
CM:"Cameroon", CN:"China", CO:"Colombia", CP:"Clipperton Island", CR:"Costa Rica", CU:"Cuba", CV:"Cape Verde", CW:"Cura\u00e7ao", CX:"Christmas Island", CY:"Cyprus", CZ:"Czechia", DE:"Germany", DG:"Diego Garcia", DJ:"Djibouti", DK:"Denmark", DM:"Dominica", DO:"Dominican Republic", DZ:"Algeria", EA:"Ceuta & Melilla", EC:"Ecuador", EE:"Estonia", EG:"Egypt", EH:"Western Sahara", ER:"Eritrea", ES:"Spain", ET:"Ethiopia", EU:"European Union", EZ:"Eurozone", FI:"Finland", FJ:"Fiji", FK:"Falkland Islands (Islas Malvinas)", 
FM:"Micronesia", FO:"Faroe Islands", FR:"France", GA:"Gabon", GB:"United Kingdom", GD:"Grenada", GE:"Georgia", GF:"French Guiana", GG:"Guernsey", GH:"Ghana", GI:"Gibraltar", GL:"Greenland", GM:"Gambia", GN:"Guinea", GP:"Guadeloupe", GQ:"Equatorial Guinea", GR:"Greece", GS:"South Georgia & South Sandwich Islands", GT:"Guatemala", GU:"Guam", GW:"Guinea-Bissau", GY:"Guyana", HK:"Hong Kong", HM:"Heard & McDonald Islands", HN:"Honduras", HR:"Croatia", HT:"Haiti", HU:"Hungary", IC:"Canary Islands", ID:"Indonesia", 
...
TN:"Tunisia", TO:"Tonga", TR:"Turkey", TT:"Trinidad & Tobago", TV:"Tuvalu", TW:"Taiwan", TZ:"Tanzania", UA:"Ukraine", UG:"Uganda", UM:"U.S. Outlying Islands", UN:"United Nations", US:"United States", UY:"Uruguay", UZ:"Uzbekistan", VA:"Vatican City", VC:"St. Vincent & Grenadines", VE:"Venezuela", VG:"British Virgin Islands", VI:"U.S. Virgin Islands", VN:"Vietnam", VU:"Vanuatu", WF:"Wallis & Futuna", WS:"Samoa", XK:"Kosovo", YE:"Yemen", YT:"Mayotte", ZA:"South Africa", ZM:"Zambia", ZW:"Zimbabwe", ZZ:"Unknown Region"}, 
LANGUAGE:{aa:"Afar", ab:"Abkhazian", ace:"Achinese", ach:"Acoli", ada:"Adangme", ady:"Adyghe", ae:"Avestan", af:"Afrikaans", afh:"Afrihili", agq:"Aghem", ain:"Ainu", ak:"Akan", akk:"Akkadian", ale:"Aleut", alt:"Southern Altai", am:"Amharic", an:"Aragonese", ang:"Old English", anp:"Angika", ar:"Arabic", ar_001:"Arabic (World)", arc:"Aramaic", arn:"Mapuche", arp:"Arapaho", ars:"Najdi Arabic", arw:"Arawak", as:"Assamese", asa:"Asu", ast:"Asturian", av:"Avaric", awa:"Awadhi", ay:"Aymara", az:"Azerbaijani", 
az_Cyrl:"Azerbaijani (Cyrillic)", az_Latn:"Azerbaijani (Latin)", ba:"Bashkir", bal:"Baluchi", ban:"Balinese", bas:"Basaa", bax:"Bamun", bbj:"Ghomala", be:"Belarusian", bej:"Beja", bem:"Bemba", bez:"Bena", bfd:"Bafut", bg:"Bulgarian", bho:"Bhojpuri", bi:"Bislama", bik:"Bikol", bin:"Bini", bkm:"Kom", bla:"Siksika", bm:"Bambara", bn:"Bangla", bo:"Tibetan", br:"Breton", bra:"Braj", brx:"Bodo", bs:"Bosnian", bs_Cyrl:"Bosnian (Cyrillic)", bs_Latn:"Bosnian (Latin)", bss:"Akoose", bua:"Buriat", bug:"Buginese", 
...
enm:"Middle English", eo:"Esperanto", es:"Spanish", es_419:"Spanish (Latin America)", es_ES:"Spanish (Spain)", es_MX:"Spanish (Mexico)", et:"Estonian", eu:"Basque", ewo:"Ewondo", fa:"Persian", fa_AF:"Persian (Afghanistan)", fan:"Fang", fat:"Fanti", ff:"Fulah", fi:"Finnish", fil:"Filipino", fj:"Fijian", fo:"Faroese", fon:"Fon", fr:"French", fr_CA:"French (Canada)", fr_CH:"French (Switzerland)", frm:"Middle French", fro:"Old French", frr:"Northern Frisian", frs:"Eastern Frisian", fur:"Friulian", fy:"Western Frisian", 
ga:"Irish", gaa:"Ga", gay:"Gayo", gba:"Gbaya", gd:"Scottish Gaelic", gez:"Geez", gil:"Gilbertese", gl:"Galician", gmh:"Middle High German", gn:"Guarani", goh:"Old High German", gon:"Gondi", gor:"Gorontalo", got:"Gothic", grb:"Grebo", grc:"Ancient Greek", gsw:"Swiss German", gu:"Gujarati", guz:"Gusii", gv:"Manx", gwi:"Gwich\u02bcin", ha:"Hausa", hai:"Haida", haw:"Hawaiian", he:"Hebrew", hi:"Hindi", hil:"Hiligaynon", hit:"Hittite", hmn:"Hmong", ho:"Hiri Motu", hr:"Croatian", hsb:"Upper Sorbian", ht:"Haitian Creole", 
hu:"Hungarian", hup:"Hupa", hy:"Armenian", hz:"Herero", ia:"Interlingua", iba:"Iban", ibb:"Ibibio", id:"Indonesian", ie:"Interlingue", ig:"Igbo", ii:"Sichuan Yi", ik:"Inupiaq", ilo:"Iloko", "in":"Indonesian", inh:"Ingush", io:"Ido", is:"Icelandic", it:"Italian", iu:"Inuktitut", iw:"Hebrew", ja:"Japanese", jbo:"Lojban", jgo:"Ngomba", jmc:"Machame", jpr:"Judeo-Persian", jrb:"Judeo-Arabic", jv:"Javanese", ka:"Georgian", kaa:"Kara-Kalpak", kab:"Kabyle", kac:"Kachin", kaj:"Jju", kam:"Kamba", kaw:"Kawi", 
kbd:"Kabardian", kbl:"Kanembu", kcg:"Tyap", kde:"Makonde", kea:"Kabuverdianu", kfo:"Koro", kg:"Kongo", kha:"Khasi", kho:"Khotanese", khq:"Koyra Chiini", ki:"Kikuyu", kj:"Kuanyama", kk:"Kazakh", kkj:"Kako", kl:"Kalaallisut", kln:"Kalenjin", km:"Khmer", kmb:"Kimbundu", kn:"Kannada", ko:"Korean", kok:"Konkani", kos:"Kosraean", kpe:"Kpelle", kr:"Kanuri", krc:"Karachay-Balkar", krl:"Karelian", kru:"Kurukh", ks:"Kashmiri", ksb:"Shambala", ksf:"Bafia", ksh:"Colognian", ku:"Kurdish", kum:"Kumyk", kut:"Kutenai", 
kv:"Komi", kw:"Cornish", ky:"Kyrgyz", la:"Latin", lad:"Ladino", lag:"Langi", lah:"Lahnda", lam:"Lamba", lb:"Luxembourgish", lez:"Lezghian", lg:"Ganda", li:"Limburgish", lkt:"Lakota", ln:"Lingala", lo:"Lao", lol:"Mongo", loz:"Lozi", lrc:"Northern Luri", lt:"Lithuanian", lu:"Luba-Katanga", lua:"Luba-Lulua", lui:"Luiseno", lun:"Lunda", luo:"Luo", lus:"Mizo", luy:"Luyia", lv:"Latvian", mad:"Madurese", maf:"Mafa", mag:"Magahi", mai:"Maithili", mak:"Makasar", man:"Mandingo", mas:"Masai", mde:"Maba", mdf:"Moksha", 
mdr:"Mandar", men:"Mende", mer:"Meru", mfe:"Morisyen", mg:"Malagasy", mga:"Middle Irish", mgh:"Makhuwa-Meetto", mgo:"Meta\u02bc", mh:"Marshallese", mi:"Maori", mic:"Mi'kmaq", min:"Minangkabau", mk:"Macedonian", ml:"Malayalam", mn:"Mongolian", mnc:"Manchu", mni:"Manipuri", mo:"Romanian (Moldova)", moh:"Mohawk", mos:"Mossi", mr:"Marathi", ms:"Malay", mt:"Maltese", mua:"Mundang", mul:"Multiple languages", mus:"Creek", mwl:"Mirandese", mwr:"Marwari", my:"Burmese", mye:"Myene", myv:"Erzya", mzn:"Mazanderani", 
...
nyo:"Nyoro", nzi:"Nzima", oc:"Occitan", oj:"Ojibwa", om:"Oromo", or:"Odia", os:"Ossetic", osa:"Osage", ota:"Ottoman Turkish", pa:"Punjabi", pa_Arab:"Punjabi (Arabic)", pa_Guru:"Punjabi (Gurmukhi)", pag:"Pangasinan", pal:"Pahlavi", pam:"Pampanga", pap:"Papiamento", pau:"Palauan", peo:"Old Persian", phn:"Phoenician", pi:"Pali", pl:"Polish", pon:"Pohnpeian", pro:"Old Proven\u00e7al", ps:"Pashto", pt:"Portuguese", pt_BR:"Portuguese (Brazil)", pt_PT:"Portuguese (Portugal)", qu:"Quechua", raj:"Rajasthani", 
rap:"Rapanui", rar:"Rarotongan", rm:"Romansh", rn:"Rundi", ro:"Romanian", ro_MD:"Romanian (Moldova)", rof:"Rombo", rom:"Romany", ru:"Russian", rup:"Aromanian", rw:"Kinyarwanda", rwk:"Rwa", sa:"Sanskrit", sad:"Sandawe", sah:"Sakha", sam:"Samaritan Aramaic", saq:"Samburu", sas:"Sasak", sat:"Santali", sba:"Ngambay", sbp:"Sangu", sc:"Sardinian", scn:"Sicilian", sco:"Scots", sd:"Sindhi", se:"Northern Sami", see:"Seneca", seh:"Sena", sel:"Selkup", ses:"Koyraboro Senni", sg:"Sango", sga:"Old Irish", sh:"Serbo-Croatian", 
shi:"Tachelhit", shi_Latn:"Tachelhit (Latin)", shi_Tfng:"Tachelhit (Tifinagh)", shn:"Shan", shu:"Chadian Arabic", si:"Sinhala", sid:"Sidamo", sk:"Slovak", sl:"Slovenian", sm:"Samoan", sma:"Southern Sami", smj:"Lule Sami", smn:"Inari Sami", sms:"Skolt Sami", sn:"Shona", snk:"Soninke", so:"Somali", sog:"Sogdien", sq:"Albanian", sr:"Serbian", sr_Cyrl:"Serbian (Cyrillic)", sr_Latn:"Serbian (Latin)", srn:"Sranan Tongo", srr:"Serer", ss:"Swati", ssy:"Saho", st:"Southern Sotho", su:"Sundanese", suk:"Sukuma", 
sus:"Susu", sux:"Sumerian", sv:"Swedish", sw:"Swahili", sw_CD:"Swahili (Congo - Kinshasa)", swb:"Comorian", syc:"Classical Syriac", syr:"Syriac", ta:"Tamil", te:"Telugu", tem:"Timne", teo:"Teso", ter:"Tereno", tet:"Tetum", tg:"Tajik", th:"Thai", ti:"Tigrinya", tig:"Tigre", tiv:"Tiv", tk:"Turkmen", tkl:"Tokelau", tl:"Tagalog", tlh:"Klingon", tli:"Tlingit", tmh:"Tamashek", tn:"Tswana", to:"Tongan", tog:"Nyasa Tonga", tpi:"Tok Pisin", tr:"Turkish", trv:"Taroko", ts:"Tsonga", tsi:"Tsimshian", tt:"Tatar", 
tum:"Tumbuka", tvl:"Tuvalu", tw:"Twi", twq:"Tasawaq", ty:"Tahitian", tyv:"Tuvinian", tzm:"Central Atlas Tamazight", udm:"Udmurt", ug:"Uyghur", uga:"Ugaritic", uk:"Ukrainian", umb:"Umbundu", und:"Unknown language", ur:"Urdu", uz:"Uzbek", uz_Arab:"Uzbek (Arabic)", uz_Cyrl:"Uzbek (Cyrillic)", uz_Latn:"Uzbek (Latin)", vai:"Vai", vai_Latn:"Vai (Latin)", vai_Vaii:"Vai (Vai)", ve:"Venda", vi:"Vietnamese", vo:"Volap\u00fck", vot:"Votic", vun:"Vunjo", wa:"Walloon", wae:"Walser", wal:"Wolaytta", war:"Waray", 
was:"Washo", wo:"Wolof", xal:"Kalmyk", xh:"Xhosa", xog:"Soga", yao:"Yao", yap:"Yapese", yav:"Yangben", ybb:"Yemba", yi:"Yiddish", yo:"Yoruba", yue:"Cantonese", za:"Zhuang", zap:"Zapotec", zbl:"Blissymbols", zen:"Zenaga", zgh:"Standard Moroccan Tamazight", zh:"Chinese", zh_Hans:"Chinese (Simplified)", zh_Hant:"Chinese (Traditional)", zh_TW:"Chinese (Taiwan)", zu:"Zulu", zun:"Zuni", zxx:"No linguistic content", zza:"Zaza"}};
goog.dom.vendor = {};
...
bcx.Popup = function() {
  ...
  this.allPolyjuiceOptions_ = null;
  this.polyjuiceProxyCurrentlySelected_ = !1;
};
...
bcx.Popup.prototype.load = function() {
  ...
  for (var inputs = goog.dom.getElementsByTagNameAndClass("input"), i = 0; i < inputs.length; i++) {
    if ("checkbox" != inputs[i].type) {
      var e$jscomp$0 = bcx.Popup.findContainer_(inputs[i]);
      e$jscomp$0 ? "radio" == inputs[i].type && goog.events.listen(e$jscomp$0, goog.events.EventType.CLICK, this.onClick_, !1, this) : bcx.Popup.log_("load", "Could not find a container for: " + inputs[i].id);
    }
  }
  goog.dom.forms.setDisabled(goog.dom.getElement("begin-polyjuice-button"), !0);
  goog.dom.forms.setDisabled(goog.dom.getElement("polyjuice-network-selector"), !0);
  goog.dom.forms.setDisabled(goog.dom.getElement("polyjuice-mobile-selector"), !0);
  goog.dom.forms.setDisabled(goog.dom.getElement("polyjuice-unburnt-selector"), !0);
  ...
  goog.style.setElementShown(goog.dom.getElement("polyjuice-controls"), !1);
  goog.style.setElementShown(goog.dom.getElement("polyjuice-load-spinner"), !1);
};
bcx.Popup.prototype.applySettings_ = function(state) {
  bcx.Popup.log_("applySettings_", "state: " + JSON.stringify(state));
  var loadPolyjuiceOptions = !1;
  switch(state.enabled) {
    ...
    case bcx.consts.POLYJUICE_CHINA:
      goog.dom.getElement("polyjuice-china").checked = !0;
      break;
    case bcx.consts.POLYJUICE:
      loadPolyjuiceOptions = goog.dom.getElement("polyjuice-proxy").checked = !0;
      break;
    case bcx.consts.BAKED_IN:
  ...
  goog.style.setElementShown(goog.dom.getElement("polyjuice-staging-label"), this.storage_.polyjuiceBackend_ === bcx.consts.POLYJUICE_BACKEND_STAGING);
  this.showPolyjuiceIfWinkyInstalled_(state.state, loadPolyjuiceOptions);
};
  ...
  if ("LABEL" != evt.target.tagName && "SELECT" != evt.target.tagName) {
    ...
    switch(id) {
      ...
      case "polyjuice-proxy":
        this.polyjuiceProxyCurrentlySelected_ || (goog.dom.getElement("polyjuice-proxy").checked = !0, this.updateHeader_(bcx.State.POLYJUICE_PENDING.popup), goog.style.setElementShown(goog.dom.getElement("polyjuice-controls"), !1), goog.style.setElementShown(goog.dom.getElement("polyjuice-load-spinner"), !0), this.loadPolyjuiceOptions_(), this.polyjuiceProxyCurrentlySelected_ = !0);
        break;
      case "polyjuice-china":
        goog.dom.getElement("polyjuice-china").checked = !0, bcx.promisechrome.sendMessage({action:bcx.consts.START_POLYJUICE_CHINA}).then(goog.bind(this.setProxy_, this, bcx.consts.POLYJUICE_CHINA)).thenCatch(goog.bind(this.onPolyjuiceError_, this, "start Polyjuice for china"));
    }
    "polyjuice-proxy" !== id && (goog.style.setElementShown(goog.dom.getElement("polyjuice-controls"), !1), goog.style.setElementShown(goog.dom.getElement("polyjuice-load-spinner"), !1), this.polyjuiceProxyCurrentlySelected_ = !1);
    "polyjuice-proxy" !== id && "polyjuice-china" !== id && (this.resetPolyjuice_(), this.endPolyjuiceSession_().then(goog.bind(this.setProxy_, this, proxy)));
  }
};
...
bcx.Popup.prototype.setProxy_ = function(proxy) {
  ...
  goog.style.setElementShown(goog.dom.getElement("polyjuice-begin-spinner"), !1);
...
bcx.Popup.prototype.showPolyjuiceIfWinkyInstalled_ = function(currentState, shouldLoadOptions) {
  goog.style.setElementShown(goog.dom.getElement("polyjuice-proxy-selector"), !1);
  goog.style.setElementShown(goog.dom.getElement("polyjuice-china-container"), !1);
  var afterSetShownCb = function() {
  };
  shouldLoadOptions && (afterSetShownCb = goog.bind(this.loadPolyjuiceOptions_, this));
  bcx.promisechrome.sendMessage({command:bcx.polyjuice.consts.HELLO}, bcx.polyjuice.consts.WINKY_APP_ID).then(goog.bind(function() {
    goog.style.setElementShown(goog.dom.getElement("polyjuice-proxy-selector"), !0);
    goog.style.setElementShown(goog.dom.getElement("polyjuice-china-container"), this.storage_.showChina_);
    afterSetShownCb();
  }, this)).thenCatch(function() {
  }).thenAlways(goog.bind(this.updateHeader_, this, currentState));
};
bcx.Popup.prototype.loadPolyjuiceOptions_ = function() {
  bcx.promisechrome.sendMessage({action:bcx.consts.GET_POLYJUICE_OPTIONS}).then(goog.bind(this.addPolyjuiceOptions_, this)).thenCatch(goog.bind(this.onPolyjuiceError_, this, "loadPolyjuiceOptions_"));
};
bcx.Popup.prototype.addPolyjuiceOptions_ = function(resp) {
  var options = resp.options;
  options && 0 != options.length && (options.forEach(function(option) {
    "isBurnt" in option || (option.isBurnt = !1);
    "isMobile" in option || (option.isMobile = !1);
  }), bcx.Popup.log_("addPolyjuiceOptions_", JSON.stringify(options)), this.allPolyjuiceOptions_ = new bcx.polyjuice.PolyjuiceOptions(options), this.addPolyjuiceCountries_(), this.addPolyjuiceNetworks_(), this.addPolyjuiceMobileOption_(), this.addPolyjuiceStealthyOption_(), this.handleStoredSelection_(), this.addPolyjuiceListeners_(), goog.style.setElementShown(goog.dom.getElement("polyjuice-controls"), this.polyjuiceProxyCurrentlySelected_), goog.style.setElementShown(goog.dom.getElement("polyjuice-begin-spinner"), 
  !1), goog.style.setElementShown(goog.dom.getElement("polyjuice-load-spinner"), !1));
};
bcx.Popup.prototype.handleStoredSelection_ = function() {
  bcx.Popup.log_("handleStoredSelection_", "");
  var storedCountryCode = this.storage_.polyjuiceCountry_;
  storedCountryCode && (goog.dom.getElement("polyjuice-country-selector").value = storedCountryCode, this.onCountryChosen_());
  var storedNetwork = this.storage_.polyjuiceNetwork_;
  storedNetwork && (goog.dom.getElement("polyjuice-network-selector").value = storedNetwork, this.onNetworkChosen_());
  var storedMobile = this.storage_.polyjuiceUseMobile_;
  storedMobile && (goog.dom.getElement("polyjuice-mobile-selector").checked = storedMobile, this.onMobileChosen_());
  var storedStealthy = this.storage_.polyjuiceUseStealthy_;
  storedStealthy && (goog.dom.getElement("polyjuice-unburnt-selector").checked = storedStealthy, this.onStealthyChosen_());
};
bcx.Popup.prototype.addPolyjuiceListeners_ = function() {
  var networkSelector = goog.dom.getElement("polyjuice-network-selector"), mobileSelector = goog.dom.getElement("polyjuice-mobile-selector"), stealthySelector = goog.dom.getElement("polyjuice-unburnt-selector"), beginButton = goog.dom.getElement("begin-polyjuice-button");
  goog.events.listen(goog.dom.getElement("polyjuice-country-selector"), goog.events.EventType.CHANGE, this.onCountryChosen_, !1, this);
  goog.events.listen(networkSelector, goog.events.EventType.CHANGE, this.onNetworkChosen_, !1, this);
  goog.events.listen(mobileSelector, goog.events.EventType.CHANGE, this.onMobileChosen_, !1, this);
  goog.events.listen(stealthySelector, goog.events.EventType.CHANGE, this.onStealthyChosen_, !1, this);
  goog.events.listen(beginButton, goog.events.EventType.CLICK, this.onBegin_, !1, this);
};
bcx.Popup.prototype.addPolyjuiceCountries_ = function() {
  var countrySelector = goog.dom.getElement("polyjuice-country-selector");
  goog.dom.forms.setDisabled(countrySelector, !0);
  goog.dom.removeChildren(countrySelector);
  var emptyCountryChoice = goog.dom.createDom("OPTION", {value:""}, "-");
  goog.dom.appendChild(countrySelector, emptyCountryChoice);
  var countriesWithNames = [];
  this.allPolyjuiceOptions_.getCountries().forEach(function(countryCode) {
  ...
  countriesWithNames.forEach(function(country) {
    var option = goog.dom.createDom("OPTION", {value:country.countryCode}, country.name);
    goog.dom.appendChild(countrySelector, option);
  });
  countrySelector.value = this.allPolyjuiceOptions_.countryCode_;
  goog.dom.forms.setDisabled(countrySelector, !1);
};
bcx.Popup.prototype.addPolyjuiceNetworks_ = function() {
  var networkSelector = goog.dom.getElement("polyjuice-network-selector");
  goog.dom.forms.setDisabled(networkSelector, !0);
  goog.dom.removeChildren(networkSelector);
  var emptyChoice = goog.dom.createDom("OPTION", {value:""}, "-");
  goog.dom.appendChild(networkSelector, emptyChoice);
  this.allPolyjuiceOptions_.getNetworks().forEach(function(network) {
    var option = goog.dom.createDom("OPTION", {value:network}, network);
    goog.dom.appendChild(networkSelector, option);
  });
  networkSelector.value = this.allPolyjuiceOptions_.networkName_;
  goog.dom.forms.setDisabled(networkSelector, !1);
};
bcx.Popup.prototype.addPolyjuiceMobileOption_ = function() {
  var mobileSelector = goog.dom.getElement("polyjuice-mobile-selector"), mobileOptions = this.allPolyjuiceOptions_.getMobileOptions();
  mobileOptions.mobile && mobileOptions.nonMobile ? (mobileSelector.checked = this.allPolyjuiceOptions_.requireMobile_, goog.dom.forms.setDisabled(mobileSelector, !1)) : (mobileSelector.checked = mobileOptions.mobile, goog.dom.forms.setDisabled(mobileSelector, !0));
};
bcx.Popup.prototype.addPolyjuiceStealthyOption_ = function() {
  var stealthySelector = goog.dom.getElement("polyjuice-unburnt-selector"), stealthyOptions = this.allPolyjuiceOptions_.getStealthyOptions();
  stealthyOptions.stealthy && stealthyOptions.nonStealthy ? (stealthySelector.checked = this.allPolyjuiceOptions_.requireStealthy_, goog.dom.forms.setDisabled(stealthySelector, !1)) : (stealthySelector.checked = stealthyOptions.stealthy, goog.dom.forms.setDisabled(stealthySelector, !0));
};
bcx.Popup.prototype.updateBeginButton_ = function() {
  goog.dom.forms.setDisabled(goog.dom.getElement("begin-polyjuice-button"), !(this.allPolyjuiceOptions_.countryCode_ || this.allPolyjuiceOptions_.networkName_));
};
bcx.Popup.prototype.onCountryChosen_ = function() {
  this.allPolyjuiceOptions_.setSelectedCountryCode(goog.dom.getElement("polyjuice-country-selector").value);
  this.addPolyjuiceNetworks_();
  this.addPolyjuiceMobileOption_();
  this.addPolyjuiceStealthyOption_();
  this.updateBeginButton_();
};
bcx.Popup.prototype.onNetworkChosen_ = function() {
  this.allPolyjuiceOptions_.setSelectedNetworkName(goog.dom.getElement("polyjuice-network-selector").value);
  this.addPolyjuiceCountries_();
  this.addPolyjuiceMobileOption_();
  this.addPolyjuiceStealthyOption_();
  this.updateBeginButton_();
};
bcx.Popup.prototype.onMobileChosen_ = function() {
  this.allPolyjuiceOptions_.setRequireMobile(goog.dom.getElement("polyjuice-mobile-selector").checked);
  this.addPolyjuiceCountries_();
  this.addPolyjuiceNetworks_();
  this.addPolyjuiceStealthyOption_();
  this.updateBeginButton_();
};
bcx.Popup.prototype.onStealthyChosen_ = function() {
  this.allPolyjuiceOptions_.setRequireStealthy(goog.dom.getElement("polyjuice-unburnt-selector").checked);
  this.addPolyjuiceCountries_();
  this.addPolyjuiceNetworks_();
  this.addPolyjuiceMobileOption_();
  this.updateBeginButton_();
};
bcx.Popup.prototype.onBegin_ = function() {
  bcx.Popup.log_("onBegin_", "");
  goog.style.setElementShown(goog.dom.getElement("polyjuice-begin-spinner"), !0);
  var selectedCountry = this.allPolyjuiceOptions_.countryCode_, selectedNetwork = this.allPolyjuiceOptions_.networkName_, selectedMobile = this.allPolyjuiceOptions_.requireMobile_, selectedUnburnt = this.allPolyjuiceOptions_.requireStealthy_;
  bcx.Popup.log_("onBegin_", "country=" + selectedCountry + " network=" + selectedNetwork + " mobile=" + selectedMobile + " unburnt=" + selectedUnburnt);
  var $jscomp$compprop0 = {};
  this.storage_.set(($jscomp$compprop0[bcx.consts.SELECTED_COUNTRY] = selectedCountry, $jscomp$compprop0));
  var $jscomp$compprop1 = {};
  this.storage_.set(($jscomp$compprop1[bcx.consts.SELECTED_NETWORK] = selectedNetwork, $jscomp$compprop1));
  var $jscomp$compprop2 = {};
  this.storage_.set(($jscomp$compprop2[bcx.consts.SELECTED_MOBILE] = selectedMobile, $jscomp$compprop2));
  var $jscomp$compprop3 = {};
  this.storage_.set(($jscomp$compprop3[bcx.consts.SELECTED_UNBURNT] = selectedUnburnt, $jscomp$compprop3));
  this.endPolyjuiceSession_();
  bcx.promisechrome.sendMessage({action:bcx.consts.START_POLYJUICE, country:selectedCountry, network:selectedNetwork, mobile:selectedMobile, burnt:!selectedUnburnt}).then(goog.bind(this.setProxy_, this, bcx.consts.POLYJUICE)).thenCatch(goog.bind(this.onPolyjuiceError_, this, "onBegin_"));
};
...
bcx.Popup.prototype.resetPolyjuice_ = function() {
  bcx.Popup.log_("resetPolyjuice_", "");
  var countrySelector = goog.dom.getElement("polyjuice-country-selector");
  ...
  goog.dom.forms.setDisabled(networkSelector, !0);
  var mobileCheckbox = goog.dom.getElement("polyjuice-mobile-selector");
  mobileCheckbox.checked = !1;
  goog.dom.forms.setDisabled(mobileCheckbox, !0);
  var stealthyCheckbox = goog.dom.getElement("polyjuice-unburnt-selector");
  stealthyCheckbox.checked = !1;
  goog.dom.forms.setDisabled(stealthyCheckbox, !0);
  goog.dom.forms.setDisabled(goog.dom.getElement("begin-polyjuice-button"), !0);
  var $jscomp$compprop4 = {};
  this.storage_.set(($jscomp$compprop4[bcx.consts.SELECTED_COUNTRY] = "", $jscomp$compprop4));
  var $jscomp$compprop5 = {};
  this.storage_.set(($jscomp$compprop5[bcx.consts.SELECTED_NETWORK] = "", $jscomp$compprop5));
  var $jscomp$compprop6 = {};
  this.storage_.set(($jscomp$compprop6[bcx.consts.SELECTED_MOBILE] = !1, $jscomp$compprop6));
  var $jscomp$compprop7 = {};
  this.storage_.set(($jscomp$compprop7[bcx.consts.SELECTED_UNBURNT] = !1, $jscomp$compprop7));
};
bcx.Popup.prototype.onPolyjuiceError_ = function(fn, err) {
  bcx.Popup.log_(fn, "Error: " + JSON.stringify(err));
