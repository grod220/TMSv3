...
// $Id: //depot/google3/corp/proxyconfig/js/pac.js#381 $
...
var isIP = {
  ...
  'local': function(ip) {
    return ip && (isInNetEx(ip, '::1/128') || isInNetEx(ip, '127.0.0.0/8'));
  },
  'prod': function(ip) {
    return ip &&
  ...
  'corp': function(ip) {
    return ip &&
        (isInNetEx(ip, '172.16.0.0/12') ||   // Corp IPv4 subnets
         isInNetEx(ip, '192.168.0.0/16') ||  // Corp DMZ subnets
         isInNetEx(ip, '100.64.0.0/10') ||   // RFC6598 block for Corp
         isInNetEx(ip, '2401:fa00::/32') || isInNetEx(ip, '2a00:79e0::/32') ||
         isInNetEx(ip, '2620:15c::/36') || isInNetEx(ip, '2620:0:1000::/40'));
  },
  'public': function(ip) {
    return (
        this.valid(ip) && !(this.corp(ip) || this.prod(ip) || this.local(ip)));
  }
};
...
function ParseHostToProxyMap(str) {
  ...
  for (var i = 0; i < strings.length; i++) {
    ...
    switch (t.length) {
      case 2:
        result.push({'host': t[0], 'proxy': t[1]});
        break;
      case 3:
        result.push({'proto': t[0].toUpperCase(), 'host': t[1], 'proxy': t[2]});
        break;
      default:
...
/** Provide mock DNS resolutions for hermetic unit tests.
 * @param {string} host - The host to resolve.
 * @returns {string}
 */
function MockDnsResolveForTests(host) {
  // Hosts that we mock unconditionally.
  switch (host) {
    ...
    case 'marcovaldo.dub.corp.google.com':
      return '172.16.17.18';
      break;
    case 'i-am-not-behind-uberproxy':
    case 'i-am-not-behind-uberproxy.corp.google.com':
    case 'i--do--not--exist--and--never--will':
    case 'i--do--not--exist--and--never--will.corp.google.com':
      return undefined;
      break;
  }
  // Mock a reasonable DNS resolution given the PAC that was requested.
  switch (params.pac) {
    case 'android':
    case 'external':
      if (IsPlainHostName(host) || IsInSiteSpecificLocation(host) ||
          IsInSiteSpecificLocation(host.replace(/\.corp\.google\.com/, '')) ||
          /^primavera/.test(host) ||  // b/19443666
          /^mes-/.test(host) ||       // b/19550118
          host == 'appcat.corp.google.com' ||
          host == 'nls-anycast.corp.google.com' ||

          // This is truly horrible, but the code will append
          // 'corp.google.com' after the initial failed resolution.
          host == 'pabf16.corp.google.com' ||

          host == 'polyc.corp.google.com' ||
          host == 'x20web-anycast.corp.google.com') {
        return undefined;
      } else if (IsCorpHostName(host)) {
        return '2a00:1450:400c:c09::81';
      }
      break;
    case 'internal':
      switch (host) {
        case 'tufte':
        case 'x20web-anycast.corp.google.com':
          return '172.16.255.48';
          break;
      }
      if (IsPlainHostName(host) || IsInSiteSpecificLocation(host) ||
          IsInSiteSpecificLocation(host.replace(/\.corp\.google\.com/, ''))) {
...
/**
 * Normalize the result of DNS resolution. Defer to mock if testing.
 * @param {string} host - The host to resolve.
 * @returns {string}
 */
function CustomDnsResolve(host) {
  if (params.is_testing) {
    return MockDnsResolveForTests(host);
  }
  var host_ip;
...
var SEND_TO_GONG_UBERPROXY = 'HTTPS up.gcp.googlers.com:443;';
...
var GOOGLE_CORP_SUBDOMAINS = [
  '.aao',      '.aar',  '.acc',        '.akl',       '.amd',  '.ame',
  '.arb',      '.ast',  '.ath',        '.atl',       '.atw',  '.aus',
  '.bej',      '.ber',  '.bev',        '.bhz',       '.bkk',  '.bld',
  '.blr',      '.bna',  '.bog',        '.bpd',       '.brd',  '.bru',
  '.bue',      '.buh',  '.cai',        '.cam',       '.cbf',  '.cgk',
  '.cha',      '.chi',  '.chl',        '.chs',       '.cib',  '.clt',
  '.cnc',      '.conf', '.cph',        '.cpk',       '.dal',  '.ddf',
  '.del',      '.det',  '.dls',        '.dtw',       '.dub',  '.dus',
  '.dxb',      '.eem',  '.enterprise', '.ffm',       '.fra',  '.ghl',
  '.grq',      '.gur',  '.ham',        '.hel',       '.her',  '.hfa',
  '.hkd',      '.hkg',  '.hot',        '.hyd',       '.i',    '.irv',
  '.jnb',      '.kci',  '.kie',        '.kir',       '.kla',  '.krk',
  '.krp',      '.kul',  '.lag',        '.lal',       '.lax',  '.lbc',
  '.ldaps',    '.lex',  '.lga',        '.lhr',       '.lim',  '.lis',
  '.lmr',      '.lnr',  '.lon',        '.lpp',       '.lul',  '.lvk',
  '.lyo',      '.mad',  '.mcast',      '.mel',       '.mev',  '.mex',
  '.mirror',   '.mnk',  '.mnl',        '.mon',       '.mor',  '.msk',
  '.msn',      '.mtv',  '.muc',        '.muo',       '.n',    '.nbo',
  '.nfs',      '.nrt',  '.nyt',        '.oib',       '.orl',  '.osa',
  '.osl',      '.ott',  '.oul',        '.pao',       '.par',  '.pco',
  '.pdx',      '.phl',  '.pit',        '.pkf',       '.plv',  '.pos',
  '.prg',      '.prh',  '.printer',    '.prom',      '.pry',  '.pst',
  '.pvu',      '.rdu',  '.res',        '.rns',       '.roam', '.rol',
  '.rom',      '.san',  '.sao',        '.sba',       '.sbo',  '.scl',
  '.sea',      '.seo',  '.sfo',        '.sha',       '.sin',  '.sjc',
  '.slc',      '.smo',  '.spb',        '.ssf',       '.sto',  '.svl',
  '.syd',      '.thn',  '.tlv',        '.tok',       '.tor',  '.tpe',
  '.tri',      '.tst',  '.twd',        '.unittests', '.vie',  '.vm',
  '.vno',      '.was',  '.wat',        '.waw',       '.wlm',  '.wro',
  '.ytspaces', '.zbc',  '.zrh'
];

DESTINATIONS_GATED_BY_UBERPROXY = [
  {
    host: /.\.googleplex\.com$/,
    comment: 'AppEngine for Corp Apps (go/googleplex-faq)'
  },
  {host: /.\.proxy\.googleprod\.com$/, bug: 'b/30638772, b/28416553'}
];

GONG_UBERPROXY_DESTINATIONS = [
  {
    host: /.\.c\.googlers\.com$/,
    bug: 'b/29408581',
    comment: 'CloudTop ("Cloud Ubiquity") hostnames'
  },
  {
    subnets: ['2607:f8b0:4800:1000::/64'],
    bug: 'b/29408581',
    comment: 'G-on-G / CloudTop ("Cloud Ubiquity") IP addresses'
  }
];

// Short forms must exclude any valid TLD.
var GOOGLE_PROD_HOSTNAMES = [
  {host: /.\.(adz|borg|cboo|dclk|jail|ls|prodz?)(\.google\.com)?$/}, {
    host: /.\.net\.google\.com$/,
    comment: 'Hydra controls, PlaNet Jupiter interface, etc',
    bug: 'b/24770593'
  },
  {
    host: /.\.googleprod\.com$/,
    comment:
        'http://docs/document/d/1A0--bq1z-y_WSF6AZT74SxVXuC_-8DfVVj-zKhvEzlI',
    bug: 'b/21771683'
  }
];
...
var FACILITIES_NETS = [
  {
    subnets: ['10.247.0.0/18', '10.247.128.0/17', '10.247.112.0/21'],
...
var CORP_APPS_TO_GO_DIRECT = [
  ...
  /** Any protocol (including e.g., WebSockets). **/
  {
  ...
  {host: /^appcat(\.corp\.google\.com)?$/, description: 'SCCM 2012 App Cat.'}, {
    host: /^nls-anycast(\.corp\.google\.com)?$/,
  ...
  /** HTTP **/
  {
    host: /^vmgws0(247|248|249|250|251|252|863|864)(\.ad\.corp\.google\.com)?$/,
  ...
  {
    host:
        /^(corp-speedtest\.i|speedtest(|2)-(mtv\.mtv|dls\.dls|eem\.eem|fra\.fra|twd\.twd|hot\.hot|cbf\.cbf))(\.corp\.google\.com)?$/,
  ...
  },
  {
    ...
    host:
        /^(aml-prod-web|rcm-dev\.(hot|vm)|wpby12\.hot|vmgol(0015|0195|235)\.vm)(|\.corp\.google\.com)$/,
    proto: HTTP,
    bug: 'b/19100292'
  },
  {
    ...
    host:
        /^mes-(x-(dev|uat|prod)|(loon|makani|iris)-prod)-app(|\.corp\.google\.com)$/,
    proto: HTTP,
    bug: 'b/19550118'
  },
  {
    description: 'GFiber Oracle Primavera P6',
    host: /^primavera-(lnp|prod|uat|sbx|dev)-app0[123](|\.corp\.google\.com)$/,
    proto: HTTP,
    bug: 'b/19443666'
  },
  {
  ...
  },
  {
    ...
    host:
        /^(nc-map1\.hot|nc-map2\.cbf|wpdc20\.hot|kpdm10\.cbf)(|\.corp\.google\.com)$/,
    proto: HTTP,
    bug: 'b/19206914'
  }
];
...
function MakeProxyInsecure(proxy_list_str) {
  return proxy_list_str.replace(/\bHTTPS(\s+)(\S+):443\b/g, 'PROXY$1$2:80');
}
...
function LooksLikeGoogleCorpProperty(request) {
  return (
      isIP.corp(request.literal_ip) || IsCorpHostName(request.host) ||
      IsInSiteSpecificLocation(request.host));
}

function LooksLikeGoogleProdProperty(request) {
  return (
      isIP.prod(request.literal_ip) ||
      RequestMatchesAny(request, GOOGLE_PROD_HOSTNAMES));
}

function LooksLikeGoogleInternalProperty(request) {
  return (
      IsPlainHostName(request.host) || LooksLikeGoogleProdProperty(request) ||
      LooksLikeGoogleCorpProperty(request));
}

// This function is only used to avoid sending traffic to sites behind
// Uberproxy via proxies like Polyjuice and the Egress Proxy. Rationales:
// * Doing so masks the actual user IP, and puts extra unnecessary pressure on
//   the proxies.
// * In the case of the Egress Proxy, the proxy's IP lends the user more
//   privileges than it should (egress proxy lives in Corp MPS, thus it is
//   given purely IP-based trust -- b/22246123 for details.
//
// Rationale for not sending more Google-owned domains (e.g., youtube.com,
// www.google.com. gmail.com) DIRECT:
...
// * Polyjuice's users must cloak to examine ads on Google.com, YouTube
//   and Blogger. See b/28784985 for more details.

function ShouldGoToDefaultProxy(host) {
  if (host === 'centrifuge-screenshot-bucket.storage.googleapis.com') {
    // See b/34210134 and b/38328500 for context.
    return false;
  }
  // Keep this in sync with
  // http://g3doc/company/teams/floonet.md#proxy-behavior If that needs to
  // diverge with what is to be sent via the Egress Proxy, then you'll need to
  // split ShouldGoToDefaultProxy() into two functions (which can probably be
  // inlined in the CanGoVia*Proxy() functions below).
  return !/.\.corp\.google\.(com|(com?\.)?([a-z][a-z]|cat))$/.test(host);
}
...
function ProcessParameters() {
  ...
  if (params.uberproxy_panic) {
    ...
    // don't want this to be an HTTPS proxy (as 'localhost' or '127.0.0.1'
    // will obviously not match the SAN/SNI list of the Uberproxy cert).
    SEND_TO_UBERPROXY = 'PROXY ' + params.uberproxy_panic + ';';
    SEND_TO_GONG_UBERPROXY = 'PROXY ' + params.uberproxy_panic + ';';
  } else if (!params.use_uberproxy_as_https_proxy) {
    // b/22713062
    SEND_TO_UBERPROXY = MakeProxyInsecure(SEND_TO_UBERPROXY);
    SEND_TO_GONG_UBERPROXY = MakeProxyInsecure(SEND_TO_GONG_UBERPROXY);
  }
}
...
function FindProxyForCorpNet(request) {
  ...
  if (request.proto == HTTP && LooksLikeGoogleCorpProperty(request)) {
    return (
        params.disable_uberproxy_fp_to_corp ? SEND_DIRECT : SEND_TO_UBERPROXY);
  }
...
function FindProxyForRequest(request) {
  ...
  if (params.map_to_proxy) {
    // Allow user customization: they can tell to send a 'host:proto'
    // combination to a given proxy; see b/27566403 for details, and
    // documentation at
    // http://g3doc/corp/proxyconfig/g3doc/user-docs/user-customizations#proxy-per-host
    var map_to_proxy = ParseHostToProxyMap(params.map_to_proxy);
    for (var i = 0; i < map_to_proxy.length; i++) {
      var obj = map_to_proxy[i];
      if (request.host != obj.host) continue;
      if (obj.proto && request.proto != obj.proto) continue;
      return obj.proxy;
    }
  }
  ...
  if (RequestMatchesAny(request, DESTINATIONS_GATED_BY_UBERPROXY)) {
    return (request.proto == HTTP ? SEND_TO_UBERPROXY : SEND_DIRECT);
  }

  if (RequestMatchesAny(request, GONG_UBERPROXY_DESTINATIONS)) {
    return (request.proto == HTTP ? SEND_TO_GONG_UBERPROXY : SEND_DIRECT);
  }

  // In internal DNS, short hostnames and *.corp hostnames can be resolve to
  // internal address that might need to be sent direct (think
  // x20web-anycast). Also, *.corp.google.com hostnames and *.${site}
  // hostnames (e.g., foo.mtv, mydesktop.dub, etc) can resolve to the
  // localhost, a fact that we want to use in later decisions.
  if (internal_dns) {
  ...
  //
  // We try not to send traffic toward "internal" Google domains via a proxy.
  // Rationales:
  //
  ...
  //     not needlessly sending also Google internal sites to it.

  // Integration of Gleeok v2 ("Polyjuice") with proxyconfig; b/26189179,
  // http://g3doc/company/teams/floonet.md#proxy-behavior.
  if (params.polyjuice_proxy && ShouldGoToDefaultProxy(request.host)) {
  ...
  if (activate_china_proxy_toggle && ShouldGoToDefaultProxy(request.host)) {
    return SEND_TO_EGRESSPROXY;
  }
