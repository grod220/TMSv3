...
SshForwardHelper.lookForSshForwardHelper = function(registeringHelper) {
  return new Promise(function(resolve) {
    console.log(UTIL_fmt('Check if SSH forward helper is installed.'));
    console.log(UTIL_fmt('Send empty message to ssh forward helper.'));
    SshForwardHelper.CONFIG.sendMessage(
        ...
        function(response) {
          ...
          console.log(UTIL_fmt('SSH forward helper is installed.'));
          // If there wasn't an error, the remote helper exists/can be run, so
          // register it, and send all messages to it.
          var forceLocal = true;
          console.log(UTIL_fmt(
              'SSH forward helper installed, forceLocal = ' + forceLocal));
