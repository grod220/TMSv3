...
function check_Gnubby(g, request, callback, opt_id) {
    ...
    if (rc == 0) {
      ...
      if (action) {
        ...
        if (thisAction == 'fwSelect') {
          var firmware = firmware_getImage(request.platformId, request.version);
          ...
          if (firmware) {
            g.lock(10, function(rc) {
              if (rc != 0) {
                ...
                var firmwareUpdate = firmware_bindUpdateFunction(
                    request.platformId, g);
                if (!firmwareUpdate) {
                  fail(-99);
                  return;
                }
                var image = UTIL_HexToBytes(firmware);
                firmwareUpdate(image,
                    function(rc) {
                      if (rc == 0) firmwareVersion = request.version;
                      fail(rc);
                    });
              }
            });
          ...
          if (rc != 0) {
            ...
            switch (rc) {
              case 509:  // RC_BANDWIDTH_EXCEEDED; overload
                ...
                break;
              case 403: {
                var msg = 'Server refused to authorize request.<br/>';
                msg += 'Make sure you have a valid machine certificate,';
                msg += ' <a href="http://go/uberproxyz/" target="_blank">';
                msg += 'go/uberproxyz</a>';
                callback(rc, msg);
              } break;
              ...
              case 401: { // failed to read user creds
                var msg = 'Authorization required.';
                msg += ' Failed to read user credentials.';
                callback(rc, msg);
              } break;
              case 200: { // install Chrome update
                var msg = 'Try updating or restarting Chrome.';
                callback(rc, msg);
              } break;
      ...
      if (firmware_needsUpdate(platformId, firmwareVersion)) {
        g.lock(10, function(rc) {
          if (rc != 0) {
            fail(rc);  // report this fresh error.
          } else {
            ...
            // return to caller.
            var latest =
                firmware_getLatestAutoUpdate(platformId, firmwareVersion);
            var image = UTIL_HexToBytes(firmware_getImage(platformId, latest));
            var firmwareUpdate = firmware_bindUpdateFunction(platformId, g);
            if (!firmwareUpdate) {
              fail(-99);
              return;
            }
            firmwareUpdate(image,
                function(rc) {
                  if (rc == 0) firmwareVersion = latest;
                  fail(rc);
                });
          }
        });
  ...
  function getFirmwareVersion() {
    g.sysinfo(function(rc, data) {
      ...
      platformId = sysInfoToPlatformId(u8);
  ...
  function getVersionData() {
    g.open(gnubbyIndex, GnubbyEnumerationTypes.VID_PID, function(rc) {
      ...
      enumeratedBy = g.which.enumeratedBy;
      g.sync(function(rc) {
        g.wink(function(rc) {
...
/**
 * Enumerate over all present fobs and apply auto-update, iff any.
 * @param {Gnubbies} gnubbies
 * @param {function()} cb callback
 */
function firmware_autoUpdateAll(gnubbies, cb) {
  gnubbies.enumerate(function(rc, devs) {
    var todo = devs.length;
    if (todo == 0) {
      cb();
      return;
    }
    for (var i = 0; i < devs.length; ++i) {
      new Gnubby().open(devs[i], GnubbyEnumerationTypes.ANY, function(rc) {
        var self = this;
        ...
        self.sync(function(rc) {
          if (rc != 0) {
            self.close();
            if (--todo == 0) cb();
            return;
          }
          self.sysinfo(function(rc, data) {
            if (rc != 0) {
              self.close();
              if (--todo == 0) cb();
              return;
            ...
            var u8 = new Uint8Array(data);
            var pid = sysInfoToPlatformId(u8);
            var ver = sysInfoToFirmwareVersionString(u8);
            var update = firmware_needsUpdate(pid, ver);

            if (update) {
              console.warn(UTIL_fmt(pid + '/' + ver + ' needs update!'));
            } else {
              console.warn(UTIL_fmt(pid + '/' + ver + ' no need update?'));
              self.close();
              if (--todo == 0) cb();
              return;
            }

            self.lock(30, function(rc) {
              if (rc != 0) {
                // lock failed; bail
                self.close();
                if (--todo == 0) cb();
              ...
              var latest = firmware_getLatestAutoUpdate(pid, ver);
              var image = UTIL_HexToBytes(firmware_getImage(pid, latest));
              var firmwareUpdate = firmware_bindUpdateFunction(pid, self);

              if (!firmwareUpdate) {
                // no fw to update to; unlock and bail
              ...
              firmwareUpdate(image, function(rc) {
                // whether successful or not: unlock and bail
                self.unlock(function() {
                  self.close();
                  if (--todo == 0) cb();
                });
