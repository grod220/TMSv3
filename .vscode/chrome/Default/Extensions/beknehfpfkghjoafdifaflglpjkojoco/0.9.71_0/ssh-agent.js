...
/**
 * Set the ssh pin
 * @param {Array<number>} pin The pin to set
 * @param {Function} cb callback.
 */
SshAgent.prototype.setPin = function(pin, cb) {
  this.handler_(
      {
        'type': 'SSH_KEYGEN',
        'pin': UTIL_BytesToHex(pin)
      },
      { 'id': chrome.runtime.id },
      function(reply) {
        cb(reply.rc);
...
SshAgent.unboundPINDialog = function(agent, msg, opt_duration) {
  var self = this;
  agent.getSSH(SshAgent.NON_ROTATING, false, function(rc, pk, ecdh) {
    if (rc !== 0) {
      self.onFailure('(failed to get ecdh. Reload?)');
      return;
    }
    var unlockSshApplet = function(pin) {
    ...
    DevicePinMap.get(UTIL_BytesToHex(pk.data()), function(rsp) {
      if (rsp.rc === 0 && rsp.pin && rsp.pin.length == 6) {
          if (opt_duration) {
            console.warn(UTIL_fmt('cannot use device pin with ecredz'));
            self.onFailure('(cannot use device pin with ecredz)');
            return;
          }
          console.log(UTIL_fmt('Using device pin'));
          unlockSshApplet(rsp.pin);
          UTIL_clear(rsp.pin);
          return;
      }
      if (rsp.rc !== 0) {
        console.warn('DevicePinMap error: ' + rsp.rc);
      }
      self.ui.getPIN(msg, function() {
...
function handleSshSignRequest(request, sendResponse) {
  ...
  state.onPINRequired = SshAgent.unboundPINDialog.bind(
      state, sshAgent, 'SSH needs Security Key password.' + ' Forgot password?' +
      ' Follow Step 2 at go/skpw');
 ...
 * @param {Function} sendResponse callback.
 */
function handleSetDevicePin(request, sendResponse) {
  if (!DevicePinMap.hasLocalStorage()) {
    console.log(UTIL_fmt('dpm local storage unavailable'));
    SshAgent.sendGenericFailure(sendResponse);
    return;
  }
  var pin = UTIL_getRandom(6);
  sshAgent.setPin(pin, function(rc) {
    if (rc !== 0) {
      console.warn(UTIL_fmt('Failed to set pin: ' + rc));
      SshAgent.sendGenericFailure(sendResponse);
      return;
    }
    sshAgent.getSSH(SshAgent.NON_ROTATING, false, function(rc, pk) {
      if (rc !== 0) {
        console.warn(UTIL_fmt('Failed to read gnubby pk'));
        SshAgent.sendGenericFailure(sendResponse);
        return;
      }
      DevicePinMap.set(UTIL_BytesToHex(pk.data()), pin, function(rc) {
        if (rc === 0) {
          var rsp = new SshBlob();
          rsp.appendByte(SSH2_AGENT_SUCCESS);
          SshAgent.sendSshBlob(sendResponse, rsp);
          ...
          console.warn(UTIL_fmt(
              'Failed to put pin in local storage. Device may be in bad state'));
          SshAgent.sendGenericFailure(sendResponse);
          return;
        }
      });
 ...
 * @param {Function} sendResponse callback.
 */
function handleHasDevicePin(request, sendResponse) {
  if (!DevicePinMap.hasLocalStorage()) {
    console.log(UTIL_fmt('dpm local storage unavailable'));
    SshAgent.sendGenericFailure(sendResponse);
    return;
  }
  var pk = request.readByteString();
  DevicePinMap.get(UTIL_BytesToHex(pk), function(rsp) {
    if (rsp.rc === 0 && rsp.pin) {
      var blob = new SshBlob();
      blob.appendByte(SSH2_AGENT_SUCCESS);
      SshAgent.sendSshBlob(sendResponse, blob);
    } else {
      SshAgent.sendGenericFailure(sendResponse);
    }
  });
 ...
 * @param {Function} sendResponse callback.
 */
function handleGetExtensions(request, sendResponse) {
  var wrapAndSend = function(extensions) {
    var rsp = new SshBlob();
    rsp.appendByte(SSH2_AGENTC_GET_EXTENSIONS_RESPONSE);
    rsp.appendSize(Object.keys(extensions).length);
    for (var ext in extensions) {
      rsp.appendByte(parseInt(ext, 10));
      rsp.appendByte(extensions[ext]);
    }
    SshAgent.sendSshBlob(sendResponse, rsp);
  };
  var extensions = {};
  chrome.runtime.getPlatformInfo(function(/** @type {{os:string}} */ info) {
    if (DevicePinMap.hasLocalStorage() && info.os == 'cros') {
      extensions[SSH2_AGENTC_SET_DEVICE_PIN] = 1;
    } else {
      extensions[SSH2_AGENTC_SET_DEVICE_PIN] = 0;
    }
    wrapAndSend(extensions);
  });
}
...
SshAgentHandler.prototype.run = function(cb) {
  ...
  switch (cmd) {
    ...
    case SSH2_AGENTC_SET_DEVICE_PIN: {
      handleSetDevicePin(req, cb);
    } break;

    case SSH2_AGENTC_HAS_DEVICE_PIN: {
      handleHasDevicePin(req, cb);
      break;
    }

    case SSH2_AGENTC_GET_EXTENSIONS_REQUEST: {
      handleGetExtensions(req, cb);
      break;
    }
    ...
    console.log(UTIL_fmt(JSON.stringify(err)));
    SshAgent.sendGenericFailure(cb);
  }
  return true;
};
