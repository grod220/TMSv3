...
Gnubby.prototype.open = function(which, opt_type, opt_cb, opt_caller) {
  ...
  if (which) {
    setCid(which);
    self.which = which;
    Gnubby.gnubbies_.addClient(/** @type {GnubbyDeviceId} */ (which), self,
        function(rc, device) {
          if (!rc) {
            self.dev = device;
            if (self.closeHook_) {
...
Gnubby.prototype.unlock = function(cb) {
  ...
  this.exchange(GnubbyDevice.CMD_LOCK, data.buffer,
...
Gnubby.prototype.sysinfo = function(cb) {
  if (!cb) cb = Gnubby.defaultCallback;
  this.exchange(GnubbyDevice.CMD_SYSINFO, new ArrayBuffer(0),
      Gnubby.NORMAL_TIMEOUT, cb);
};
...
Gnubby.prototype.wink = function(cb) {
  if (!cb) cb = Gnubby.defaultCallback;
  this.exchange(GnubbyDevice.CMD_WINK, new ArrayBuffer(0),
      Gnubby.NORMAL_TIMEOUT, cb);
};
...
Gnubby.prototype.reset = function(cb) {
  if (!cb) cb = Gnubby.defaultCallback;
  this.exchange(GnubbyDevice.CMD_ATR, new ArrayBuffer(0),
...
Gnubby.prototype.usb_test = function(args, cb) {
  ...
  this.exchange(GnubbyDevice.CMD_USB_TEST, u8.buffer,
      Gnubby.NORMAL_TIMEOUT, cb);
};
