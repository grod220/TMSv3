...
function getOriginFromUrl(url) {
  ...
  // Optionally use a static origin string for Tin testing; otherwise, the RPCs
  // won't be consistent on Tap/Guitar machines due to different origins.
  if (hasGoogleDomainName_(origin) && USE_FAKE_STATIC_ORIGIN_FOR_TESTING) {
...
/**
 * Checks whether the domain name portion of the origin string ends with
 *   .google.com.
 *
 * @param {string} origin The origin to check.
 * @return {boolean} Whether the domain name portion of the origin string ends
 *     with .google.com.
 * @private
 */
function hasGoogleDomainName_(origin) {
  var re = new RegExp('^https?:\/\/([^:/])*/?');
  var domain = re.exec(origin);
  if (domain === null) {
    return false;
  }
  return domain[0].endsWith('.google.com');
}
