...
/**
  ...
  */
function _testWebMessage(request, url, opt_callback) {
  var callback = opt_callback || _defaultCallback;
  messageHandler(
      request,
      /** @type {!MessageSender} */ ({
...
function testEnroll(nonGoogle, opt_callback) {
  var appId = nonGoogle ? GoogleCorpIndividualAttestation.GOOGLE_CORP_APP_ID +
      '?testingNonCorpAppId': GoogleCorpIndividualAttestation.GOOGLE_CORP_TEST_APP_ID;
  var url = nonGoogle ? 'https://security.google.com/': GoogleCorpIndividualAttestation.GOOGLE_CORP_TEST_APP_ID;
  var request = {
  ...
  _testWebMessage(request, url, opt_callback);
}
...
function testSingleSigner(opt_instances, opt_timeoutSeconds) {
  var DEFAULT_GNUBBY_INSTANCES = 1;
    ...
    return function() {
      var s = new SingleGnubbySigner(
          ...
          function(result) {
            ...
            console.log(UTIL_fmt(JSON.stringify(result)));
            if (result.gnubby) {
              result.gnubby.close();
            }
          },
  ...
  function testLoop(keyHandle) {
    ...
    console.log(UTIL_fmt(JSON.stringify(challenge)));
  ...
  function enumerated(rc, gnubbyIds) {
    ...
    g.open(gnubbyId, GnubbyEnumerationTypes.ANY, opened);
...
/**
 * Print attestation certificate to the console.
 * @param {(boolean|undefined)} nonGoogle Whether the appId is for Google.com.
 * @param {function(Object)=} opt_cb Optional callback to parse/log attestation cert.
 */
function getAttestationCertificate(nonGoogle, opt_cb) {
  function printAttestationCert(msg) {
    ...
    var LINE_LENGTH = 60;
    var certAsBase64 = window.btoa(UTIL_BytesToString(msg.cert));
    var pemStr = '-----BEGIN CERTIFICATE-----\n';
    for (var i = 0; i < certAsBase64.length; i += LINE_LENGTH) {
      pemStr += certAsBase64.substr(i, LINE_LENGTH) + '\n';
    }
    pemStr += '-----END CERTIFICATE-----';
    console.log(UTIL_fmt('attestation certificate:\n' + pemStr));
  }
