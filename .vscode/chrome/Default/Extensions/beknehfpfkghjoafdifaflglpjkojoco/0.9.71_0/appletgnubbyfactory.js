...
/**
 * Called after enrollment to ensure the ssh-applet is not present
 ...
 */
AppletVerifyingGnubbyFactory.prototype.postEnrollAction =
    function(gnubby, appIdHash, cb) {
  if (appIdHash !== GoogleCorpIndividualAttestation.GOOGLE_CORP_APP_ID_HASH) {
    cb(DeviceStatusCodes.OK_STATUS);
    return;
  }
  var removeSshApplet = () => {
    this.showAppletUpdateWindow_();
    gnubby.appletUpdate('ssh&v2&delete', true, (rc) => {
      this.hideAppletUpdateWindow_(rc, true);
      if (rc !== 0) {
        console.log(UTIL_fmt('failed to remove ssh applet: ' + rc));
      }
      // Fail open
      cb(DeviceStatusCodes.OK_STATUS);
    });
  ...
  gnubby.lock(10, () => {
    gnubby.selectSSH((rc, data) => {
      if (rc == DeviceStatusCodes.FILE_NOT_FOUND_STATUS) {
        gnubby.unlock(() => cb(DeviceStatusCodes.OK_STATUS));
      } else {
        console.log(UTIL_fmt('removing ssh applet'));
        gnubby.unlock(removeSshApplet);
      }
    });
  });
};
