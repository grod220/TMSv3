...
Gnubby.redactRequestLog = function(frame) {
  ...
  if (!hid) {
    ...
    // Likely HID frame; sniff content and redact if needed.
    // Overmatching is fairly unlikely yet harmless.
    if (frame[4] == 0x83 && frame[5] == 0x00 && frame[6] == 0x0b &&
        frame[7] == 0x00 && frame[8] == Gnubby.SSH_PIN_GENKEY &&
        frame[9] == 0x00 && frame[10] == 0x00 && frame[11] == 0x06) {
      var result = UTIL_BytesToHex(frame.subarray(0, 12)) + 'XXXXXXXXXXXX' +
          UTIL_BytesToHex(frame.subarray(12 + 6));
      return result;
    }
  }
 ...
 *
 * @param {Array|Uint8Array} ecdh handshake curve point.
 ...
 * Re-initialize SSH
 *
 * Signal to aware fw versions that we'd like to reset to the no PIN state.
 * However, not all fw versions have such a signal and lock out is the next best
 * approximation.
 *
 * Hence we send repeated unlock attemps with wrong pin to get a lockout, which
 * works for all firmware versions.
 * But we signal our reset desire with a recognizable sequence of all zeros, so
 * that aware fw versions can revert to no PIN state.
 *
 * @param {Function} cb callback.
 */
Gnubby.prototype.reinitializeSSH = function(cb) {
  var self = this;

  var done = function(rc) {
    self.unlock(function() { cb(rc); });
  };

  self.lock(10, function(rc) {
    if (rc != 0) { done(rc); return; }
    self.selectSSH(function(rc) {
      if (rc != 0) { done(rc); return; }
      // Use 16 zeros as unlock encrypted pin.
      // This is used to signal to aware fw versions that what
      // we really want here to erase the SSH pin, not lock out per se.
      var allzeros = new Array(16);
      self.getProtectedPublicKey(allzeros, 0, false,
          function(rc, data) {
            if (rc != 0) { done(0); return; }
            var ecdh = new Uint8Array(data).slice(0, 65);
            var attempts = 12;
            var tryunlock = function() {
              if (--attempts <= 0) { done(0); return; }
              self.unlockProtectedKey(ecdh, allzeros, 0, tryunlock);
            };
            tryunlock();
          });
    });
