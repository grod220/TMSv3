...
Gnubby.prototype.appletUpdate = function(which, force, cb) {
  ...
  if (chrome.runtime.id == 'dlfcjilkjfhdnfiecknlnddkmmiofjbg') {
    // Gnubbyd-dev can use bleeding edge servers.
    servers = [
  ...
  function xhr_pump(code, rsp_ab) {
    ...
    xhr_send(
        XHR,
        server + '/ga-update?sid=' + sid + '&data=' + data + '&seq=' + seq);
  }
  ...
  function locked(code) {
    if (code == Gnubbyd.RSP_OK) {
      ...
      xhr_send(XHR, server + '/ga-update?sid=' + sid);
  ...
  function tryRetrieveToken(cb) {
    if (!chrome.identity) {
      cb();
      return;
    }
    chrome.identity.getAuthToken({interactive: false}, function(auth_token) {
  ...
  // First check what type of gnubby we're dealing with.
  // Haven fobs have no applets, but we interpret the applet update request
  // to mean a request to reset the SSH pin instead.
  self.sysinfo(function(rc, data) {
    if (rc == 0) {
      var u8 = new Uint8Array(data);
      if (u8.length >= 8 && u8[8] == 0x03) {
        // Haven gnubby. Assume request is to wipe the SSH state.
        self.reinitializeSSH(cb);
        return;
      }
    }
    tryRetrieveToken(startXHR);
  });
};
